...
OSCalculate <- function(sheet_individual, type){
  ...
  if(isTRUE(unique(sheet_individual$'死亡') == "是")){
    ...
    if(length(which(names(sorttimes) %in% '死亡时间')) > 0){
      ...
      most_OS <- as.numeric(difftime(as.POSIXct(sheet_individual$'随访时间'[dim(sheet_individual)[1]]), min(as.POSIXct(sorttimes[grep('入院日期', names(sorttimes))]), as.POSIXct(sorttimes[grep('手术日期',  names(sorttimes))])), units="days"))
      if(type == "max"){
        OS <- most_OS
      }
      if(type == "min"){
